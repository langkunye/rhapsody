...
public class InsertStringToPdfComand {
	...
	public static void main(String[] args) {
		...
		// parse CSV
		String[][] csvData = CSVParser.parse(csvFilePath);
		if (csvData.length < 4) {
		...
		for (int i = 4; i < csvData.length; i++) {
			...
			try {
				...
				//PDFont font = PDType0Font.load(doc, new File("C:/Windows/Fonts/MSPRGOT.TTF"));
				PDFont font = PDType0Font.load(doc, new File(fontPath));
			    PDPage page = doc.getPage(0);
			    PDPageContentStream pdfSteam = new PDPageContentStream(
	...
	// insert line
	private static void printLine(PDDocument doc, PDPageContentStream stream,
			List<StringInfo> strInfoList, PDFont font)
			throws IOException {
